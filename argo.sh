#!/bin/bash
CT8PASSWORD=${CT8PASSWORD:-'password'}

USERNAME=$(whoami)
UUID=${UUID:-'de04add9-5c68-8bab-950c-08cd5320df18'}
USERNAME_DOMAIN=$(whoami | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9-]//g')
WORKDIR="/home/${USERNAME}/domains/${USERNAME_DOMAIN}.ct8.pl/public_nodejs"
...
set_domain_dir() {
    local DOMAIN="${USERNAME_DOMAIN}.ct8.pl"
...
generate_dotenv() {
    ...
    echo "CT8PASSWORD='${CT8PASSWORD}'" > ${WORKDIR}/.env
    cat >> ${WORKDIR}/.env << EOF
UUID=${UUID}
...
get_app() {
    ...
    wget -t 10 -qO ${WORKDIR}/app.js https://raw.githubusercontent.com/esureos/sb-for-serv00/ct8/app.js
    if [ $? -ne 0 ]; then
        echo "app.js 下载失败！请检查网络情况！"
        exit 1
    fi
    echo "正在下载 package.json 请稍候..."
    wget -t 10 -qO ${WORKDIR}/package.json https://raw.githubusercontent.com/esureos/sb-for-serv00/main/package.json
    if [ $? -ne 0 ]; then
...
get_certificate() {
    ...
    local CERT_OUTPUT=$(env CT8PASSWORD="$CT8PASSWORD" expect << EOF
spawn devil ssl www get "${IP_ADDRESS}" "${DOMAIN}"
expect "Password:"
send "\$env(CT8PASSWORD)\r"
expect eof
catch wait result
puts "\nResult: \$result\n"
EOF
...
hysteria2://${UUID}@${HOST}:${PORT1}/?sni=${DOMAIN}#🇵🇱PL-hy2-k0baya-ct8
        
----------------------------
        
tuic5配置：
        
tuic://${UUID}:${UUID}@${HOST}:${PORT2}//?congestion_control=bbr&udp_relay_mode=native&sni=${DOMAIN}&alpn=h3#🇵🇱PL-tuic5-k0baya-ct8
        
----------------------------
        
vless配置：
        
vless://${UUID}@${HOST}:${PORT3}?alpn=h3&flow=&host=${DOMAIN}&path=/ct8-vl&security=tls&sni=${DOMAIN}&type=ws#🇵🇱PL-Vless-k0baya-ct8s
        
----------------------------
        
vless配置：
        
vless://${UUID}@${SPAREHOST1}:${PORT3}?alpn=h3&flow=&host=${DOMAIN}&path=/ct8-vl&security=tls&sni=${DOMAIN}&type=ws#🇵🇱PL-Vless-k0baya-ct8c

----------------------------
        
vless配置：
        
vless://${UUID}@${SPAREHOST2}:${PORT3}?alpn=h3&flow=&host=${DOMAIN}&path=/ct8-vl&security=tls&sni=${DOMAIN}&type=ws#🇵🇱PL-Vless-k0baya-ct8w

----------------------------
        
vless配置：
        
vless://${UUID}@upos-sz-mirrorcf1ov.bilivideo.com:443?alpn=h3&flow=&host=\${ARGO_DOMAIN}&path=/ct8-vl&security=tls&sni=\${ARGO_DOMAIN}&type=ws#🇵🇱PL-Argo-Vless-k0baya-ct8

*******************************************
EOF

echo \$(echo -n "hysteria2://${UUID}@${HOST}:${PORT1}/?sni=${DOMAIN}#🇵🇱PL-hy2-k0baya-ct8

tuic://${UUID}:${UUID}@${HOST}:${PORT2}//?congestion_control=bbr&udp_relay_mode=native&sni=${DOMAIN}&alpn=h3#🇵🇱PL-tuic5-k0baya-ct8

vless://${UUID}@${HOST}:${PORT3}?alpn=h3&flow=&host=${DOMAIN}&path=/ct8-vl&security=tls&sni=${DOMAIN}&type=ws#🇵🇱PL-Vless-k0baya-ct8

vless://${UUID}@${SPAREHOST1}:${PORT3}?alpn=h3&flow=&host=${DOMAIN}&path=/ct8-vl&security=tls&sni=${DOMAIN}&type=ws#🇵🇱PL-Vless-k0baya-ct8

vless://${UUID}@${SPAREHOST2}:${PORT3}?alpn=h3&flow=&host=${DOMAIN}&path=/ct8-vl&security=tls&sni=${DOMAIN}&type=ws#🇵🇱PL-Vless-k0baya-ct8

vless://${UUID}@upos-sz-mirrorcf1ov.bilivideo.com:443?alpn=h3&flow=&host=\${ARGO_DOMAIN}&path=/ct8-vl&security=tls&sni=\${ARGO_DOMAIN}&type=ws#🇵🇱PL-Argo-Vless-k0baya-ct8" | base64 ) > ${WORKDIR}/sub
  }
...
generate_config
[ -e ${WORKDIR}/cert.crt ] && [ -e ${WORKDIR}/private.key ] && echo "请访问 https://${USERNAME_DOMAIN}.ct8.pl/status 获取服务端状态, 当 sing-box 和 cloudflared 正常运行后，访问 https://${USERNAME_DOMAIN}.ct8.pl/list 获取配置" && exit 0
